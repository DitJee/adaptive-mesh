...
C 
      ...
      OPEN(UNIT= 8,FILE=FILNAM//'.KK'//CV,STATUS='UNKNOWN')
      ...
      OPEN(UNIT=19,FILE=FILNAM//'.SHO',STATUS='UNKNOWN')
C
...
C
C *** ADD ADITIONAL TRIANGLE IN ORDER TO MAKE THE DOMAIN COVERED
...
C
      ...
      WRITE(19,*) ' *** FILLING ISIDE FOR THE GENERATED MESH'
...
C
      WRITE(19,*) ' *** WHAT SHALL WE DO NOW ?'
      WRITE(19,*) '     0 - QUIT'
      WRITE(19,*) '     1 - PLOT THE MESH'
      WRITE(19,*) '     2 - SMOOTH THE MESH'
      WRITE(19,*) '     3 - SWAP DIAGONALS'
      WRITE(19,*) '     4 - EAT 3 S '
      WRITE(19,*) '     5 - AREA CHECK/OUTPUT NO OF NODES AND ELEMENTS'
      WRITE(19,*) '     6 - GET THE RE-START FILE'
      READ (7,*)  IWHAT
      WRITE(6,*)  IWHAT
      WRITE(19,*)  IWHAT
...
40000 CONTINUE
      ...
      WRITE(19,*) ' ENTER NUMBER OF SMOOTHING LOOPS'
      READ (7,*) NSMOO
      WRITE(6,*) NSMOO
      WRITE(19,*) NSMOO
...
C
20000 CONTINUE
C     WRITE (9,20002) (IP,(COOR(IJ,IP),IJ=1,2),(STREC(IJ,IP),
C    .                 IJ=1,4),IP=1,NODE)
C
    1 FORMAT(20A4)
C   3 FORMAT(I5,1P6E15.7)
C  31 FORMAT(I5,1P2E15.7,4I5) 
C   5 FORMAT(' INITIAL BOUNDARY/FIXED NODES')
C   6 FORMAT((16I5))
C   7 FORMAT(' INPUT AND GENERATED NODES')
C   8 FORMAT(' DO YOU WANT TO SEE GENERATED NODES? 1 YES 0 NO')
...
C
 ...
 1006 CONTINUE
C
C *** READ THE VALUE OF THE UNKNOWNS FOR INTEPOLATION
C
      READ(9,1) TEXT
      DO 1007 IP=1,NPOIG
...
C
     ...
     1                  LCOOR,NREGI)
C
C *** THIS SUBROUTINE ADD TRIANGLES TO THE BACGROUND GRID IN ORDER
...
C
      ...
      WRITE(19,*) ' ENTER EXPANSION FACTOR'
      READ (7,*)  EXPA
      WRITE(6,*)  EXPA
      WRITE(19,*)  EXPA
C
...
C
C *** THIS  SUBROUTINE  COMPUTES   THE   POINT   CURVATURE
C     FOR A BEZIER QUADRATIC SPLINE GIVEN THE VALUE OF THE
C     T PARAMETER
C
...
C
C *** THIS SUBROUTINE PLOTS A QUADRATIC  BEZIER  SPLINE  AS  A
C     COLLECTION OF NPART STRAIGHT LINES
C
...
C
     ...
     1                   COORG,DELTA,ILAST,IGEOM) 
C
C     REAL COORG(2,NPOIG),DELTA(4,NPOIG)
...
C
      ...
      WRITE(19,*) 'AW FOR RE-ORDERING, (AW=1.:ALWAYS,AW=LARGE:NEVER)' 
      READ (7,*) AW 
      WRITE(6,*) AW 
      WRITE(19,*) AW 
C
...
C
      ...
      XTEMPF=XBARF+XCBF
      YTEMPF=YBARF+YCBF
C     XTEMP=XTR(XTEMPF,YTEMPF,ANX,ANY,ALPH)
C     YTEMP=YTR(XTEMPF,YTEMPF,ANX,ANY,ALPH)
...
C
      ...
      XKEN=COOR(1,KEN)
      YKEN=COOR(2,KEN)
C     XKENF=XBA(XKEN,YKEN,ANX,ANY,ALPH) 
C     YKENF=YBA(XKEN,YKEN,ANX,ANY,ALPH) 
...
C      CNX=COOR(1,KI)
C      CNY=COOR(2,KI)
C      CNXF=XBA(CNX,CNY,ANX,ANY,ALPH)
C      CNYF=YBA(CNX,CNY,ANX,ANY,ALPH)
C      WX1=CNXF-XTEMPF
C      WY1=CNYF-YTEMPF
C      D1=SQRT(WX1*WX1+WY1*WY1)
C      IF(D1.LT.0.5*DSIDE) GOTO 720
C      DO 575 I=1,INUM-K
C      J=INUM-I
C      NEAR(J+1)=NEAR(J)
C      HOWF(J+1)=HOWF(J)
...
C
C      XPF=XBA(XP,YP,ANX,ANY,ALPH)
C      YPF=YBA(XP,YP,ANX,ANY,ALPH)
C      D1=SQRT((XPF-XN1F)**2+(YPF-YN1F)**2)
C      D2=SQRT((XPF-XNF)**2+(YPF-YNF)**2)
C      DI=AMAX1(D1,D2)
C      IF(DI.LT.1.5*DSIDE) GOTO 603
  601  CONTINUE
C
C      WE ARE IN TROUBLE !!!!!
...
C      SEE IF THIS CONNECTION IS POSSIBLE
C
C      CALL POSSIBLE(KN1,KN,KP,XN1,YN1,XN,YN,XP,YP,IREG,NONF,
C    1               NONR,NPFRONT,NQFRONT,NREGI,COOR,IYON)
C
C      IF(IYON.EQ.0) GOTO 701 
C      IF(ANGI.GT.0.7853981) GOTO 603
C
C         CHECK NOW THE SIZE
C
C      XPF=XBA(XP,YP,ANX,ANY,ALPH)
...
C
       ...
       WRITE(19,*) ' CANNOT FIND THE CONNECTIVITY'
C
C      TRY ANOTHER SIDE
C
       DISW1=1.01*ALEN1F
...
C
      ...
      Y2=COORD(2,I4)
      Y3=COORD(2,I3)
C     IF(DETER(X1,Y1,X2,Y2,X3,Y3).LE.0.0) GOTO 2000
...
C
C     -----THIS SUB IS AN OUTPUT-FOR-INPUT ROUTINE
C
...
C**    5 STATEMENTS AND PRINT OUT NODAL UNKNOWNS DIRECTLY: (20 JULY '88)
C**
...
C
    ...
    6 FORMAT(' LAST VALUES FOR THE UNKNOWNS :')
...
C
       ...
       WRITE(19,*) DELKMMA,DELKMMI,DELSCA
...
C     WRITE (9,20002) (IP,(COORG(IJ,IP),IJ=1,2),(DERI2(IJ,IP),
C    .                 IJ=1,4),IP=1,NPOIG)
20002 FORMAT(I8,2X,1P6E15.7)
C
...
c      Y=COORG(2,IPOIN)
c      RAD=SQRT(X*X+Y*Y)
c      RMAX=1.7+0.1*ABS(Y)/RAD
c      DELRAJ=DELSCA+(RAD-RMIN)*3.0*DELSCA/(RMAX-RMIN)
...
c      IF((RAD.LT.RMAX).AND.(DELRAJ.LT.DELKM)) DELKM=DELRAJ
       HELP(1,IPOIN)=DELKM
       HELP(2,IPOIN)=DELT2
 1202  CONTINUE
C
...
c      RAD=SQRT(X*X+Y*Y)
c      RMAX=1.7+0.1*ABS(Y)/RAD
c      DELRAJ=1.0+(RAD-RMIN)*2.0/(RMAX-RMIN)
c      IF(DELRAJ.LT.1.0) DELRAJ=1.0
c      IF(DELRAJ.GT.3.0) DELRAJ=3.0
       IF(STRE.GT.STR) STRE=STR
       IF(STRE.LT.1.0) STRE=1.0
c      IF((DELRAJ.GT.1.0).AND.(DELRAJ.LT.3.0)) STRE=DELRAJ
       DELTA(2,IPOIN)=STRE
 1402  CONTINUE
C      WRITE (9,20003)
20003  FORMAT(' DELTA',/)
C      WRITE (9,20002) (IP,(COORG(IJ,IP),IJ=1,2),(DELTA(IJ,IP),
C    .                  IJ=1,4),IP=1,NPOIG)
C
...
C
C     -----THIS SUB EVALUATES N,X & N,Y FOR EACH ELEMENT
